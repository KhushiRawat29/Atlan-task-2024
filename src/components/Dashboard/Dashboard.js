...
const Dashboard = ({ user, itineraries, setItineraries }) => {
  // Fallback user object for demonstration purposes
  const fallbackUser = {
    username: 'Guest',
  ...
  // Use the fallback user if the actual user is not available
  ...
  useEffect(() => {
    const fetchItineraries = async () => {
      try {
        const response = await getItineraries(currentUser.userId, currentUser.token);
        if (Array.isArray(response)) {
          setItineraries(response);
        } else {
          console.error('Expected an array of itineraries');
